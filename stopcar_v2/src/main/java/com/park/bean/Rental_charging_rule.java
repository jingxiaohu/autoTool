...
public class Rental_charging_rule implements Cloneable , Serializable{
    ...
    public long rcr_id;//BIGINT    主键ID
    public long pi_id;//BIGINT    停车场主键ID（外键）
    public int start_price;//INT    起步价(RMB单位分)
    public int start_time;//INT    起步价(RMB单位分)
    public int charging;//INT    计费价(RMB单位分)
    public int charging_time;//INT    计费时长(分钟)
    public int month_price;//INT    包月价格(单位分)
    public int month_time;//INT    包月时长(天)
    public String permit_time="";//VARCHAR    准入时间段（17:00-08:30）
    public String timeout_info="";//VARCHAR    超时费率(首停2小时3元，之后1元/小时)
    public int rcr_type;//INT    停车类型0：普通车位停车1：时间段包月停车
    public int rcr_state;//INT    是否有效0：有效1：无效
    public int rcr_discount;//INT    是否可以使用优费券:0：可以使用1：无法使用
    public String car_displacement="";//VARCHAR    车辆排量
    public int car_type;//INT    车牌类型0：未知车牌:、1：蓝牌小汽车、2：:黑牌小汽车、3：单排黄牌、4：双排黄牌、5：警车车牌、6：武警车牌、7：个性化车牌、8：单排军车牌、9：双排军车牌、10：使馆车牌、11：香港进出中国大陆车牌、12：农用车牌、13：教练车牌、14：澳门进出中国大陆车牌、15：双层武警车牌、16：武警总队车牌、17：双层武警总队车牌
    public int car_code_color;//INT    车牌颜色0：未知、1：蓝色、2：黄色、3：白色、4：黑色、5：绿色
    public int is_time_bucket;//INT    是否按时间段收费0:不按时间段收费1：按时间段收费
    public String time_bucket="";//VARCHAR    时间段收费例如：白天9：00-12：00每小时2元
    public java.util.Date ctime=new java.util.Date();//DATETIME    创建时间
    public java.util.Date utime=new java.util.Date();//DATETIME    更新时间
    public String rcr_md5="";//VARCHAR    规则MD5校验pi_id+start_price+start_time+charging+charging_time+rcr_type+rcr_discount+car_type+car_sign_type+is_time_bucket
    public int is_default;//INT    是否是默认规则0:不是1：是
    public String area_code="";//VARCHAR    省市区代码
