...
public class User_moneyback implements Cloneable , Serializable{
    ...
    //public static String[] carrays ={"um_id","ui_id","order_id","pi_id","ctime","utime","um_money","car_code","run_url","um_state","check_state","admin_userid","note","area_code","type","content","is_rent","check_content"};

    public long um_id;//BIGINT    主键ID
    public long ui_id;//BIGINT    用户ID
    public String order_id="";//VARCHAR    停车下订单ID
    public long pi_id;//BIGINT    场地表主键ID
    public java.util.Date ctime=new java.util.Date();//DATETIME    创建时间
    public java.util.Date utime=new java.util.Date();//DATETIME    更新时间
    public int um_money;//INT    退款金额(单位分)
    public String car_code="";//VARCHAR    退款车牌号
    public String run_url="";//VARCHAR    用户行驶证图片
    public int um_state;//INT    退款状态0：未退款1：已退款
    public int check_state;//INT    审核状态0：未审核1：审核未通过2:审核通过
    public long admin_userid;//BIGINT    审核人后台管理表主键ID
    public String note="";//VARCHAR    
    public String area_code="";//VARCHAR    区域地址编码
    public int type;//INT    申诉类型0：临停扣款问题1：预约超时扣费问题2：其它
    public String content="";//VARCHAR    申诉原因
    public int is_rent;//INT    是否是租赁订单0：临停订单1：预约临停订单2：租赁包月订单
