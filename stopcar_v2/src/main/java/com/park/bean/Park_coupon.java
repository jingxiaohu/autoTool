...
public class Park_coupon implements Cloneable , Serializable{
    ...
    //public static String[] carrays ={"id","name","money","discount","high_money","end_time","use_range","pc_type","ctime","utime","note"};

    public long id;//BIGINT    主键ID
    public String name="";//VARCHAR    
    public int money;//INT    优惠券金额(单位分)
    public double discount;//FLOAT    折扣券折数（单位实数）
    public int high_money;//INT    最高抵扣金额
    public java.util.Date end_time=new java.util.Date();//DATETIME    有效期到期时间
    public int use_range;//INT    使用范围0：爱泊车场通用1：其它车场使用
    public int pc_type;//INT    优惠券类型0:金额券1：折扣券
    ...
    public static Park_coupon newPark_coupon(long id, String name, int money, double discount, int high_money, java.util.Date end_time, int use_range, int pc_type, java.util.Date ctime, java.util.Date utime, String note) {
