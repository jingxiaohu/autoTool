...
public class Parkinfo_partnerDao extends BaseDao{
    ...
    Logger log = Logger.getLogger(Parkinfo_partnerDao.class);



    public  String TABLE = "parkinfo_partner";

    public  String TABLENAME = "parkinfo_partner";
    ...
    public  String[] carrays ={"id","pi_id","pu_id","pu_nd","ctime","utime","is_effect","area_code","note"};
    public  String coulmns ="id,pi_id,pu_id,pu_nd,ctime,utime,is_effect,area_code,note";
    public  String coulmns2 ="pi_id,pu_id,pu_nd,ctime,utime,is_effect,area_code,note";
    ...
    public int insert(Parkinfo_partner bean, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (pi_id,pu_id,pu_nd,ctime,utime,is_effect,area_code,note) VALUES (:pi_id,:pu_id,:pu_nd,:ctime,:utime,:is_effect,:area_code,:note)";
    ...
    //添加数据
    public int insert2(Parkinfo_partner bean) throws SQLException{
        return insert2(bean, TABLENAME);
    }

    //添加数据
    public int insert2(Parkinfo_partner bean, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (id,pi_id,pu_id,pu_nd,ctime,utime,is_effect,area_code,note) VALUES (:id,:pi_id,:pu_id,:pu_nd,:ctime,:utime,:is_effect,:area_code,:note)";
            SqlParameterSource ps = new BeanPropertySqlParameterSource(bean);
    ...
    public int[] insert(final List<Parkinfo_partner> beans, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (pi_id,pu_id,pu_nd,ctime,utime,is_effect,area_code,note) VALUES (?,?,?,?,?,?,?,?)";
    ...
    public List<Parkinfo_partner> selectAll(String TABLENAME2) {
        String sql;
        try{
            sql = "SELECT id,pi_id,pu_id,pu_nd,ctime,utime,is_effect,area_code,note FROM "+TABLENAME2+" ORDER BY id";
    ...
    public List<Parkinfo_partner> selectLast(int num ,String TABLENAME2) {
        String sql;
        try{
            sql = "SELECT id,pi_id,pu_id,pu_nd,ctime,utime,is_effect,area_code,note FROM "+TABLENAME2+" ORDER BY id DESC LIMIT "+num+"" ;
            return _np.getJdbcOperations().query(sql, new BeanPropertyRowMapper<Parkinfo_partner>(Parkinfo_partner.class));
        }catch(Exception e){
            //createTable(TABLENAME2);
            log.error(e);
    ...
    public List<Parkinfo_partner> selectGtKey(long id, String TABLENAME2) {
        String sql;
        try{
            sql="SELECT id,pi_id,pu_id,pu_nd,ctime,utime,is_effect,area_code,note FROM "+TABLENAME2+" WHERE id>:id";
            Map<String,Object> param = new HashMap<String,Object>();
            param.put("id", id);
    ...
    public Parkinfo_partner selectByKey(long id, String TABLENAME2) {
        ...
        try{
            ...
            Map<String,Object> param = new HashMap<String,Object>();
            param.put("id", id);
            return (Parkinfo_partner)_np.queryForObject(sql, param, new BeanPropertyRowMapper<Parkinfo_partner>(Parkinfo_partner.class));
        }catch(Exception e){
            //createTable(TABLENAME2);
            log.error(e);
            return null;
    ...
    public List<Parkinfo_partner> selectByPage(int begin, int num, String TABLENAME2) {
        try{
            String sql;
            sql = "SELECT id,pi_id,pu_id,pu_nd,ctime,utime,is_effect,area_code,note FROM "+TABLENAME2+" LIMIT "+begin+", "+num+"";
            return _np.getJdbcOperations().query(sql,new BeanPropertyRowMapper<Parkinfo_partner>(Parkinfo_partner.class));
        }catch(Exception e){
            //createTable(TABLENAME2);
            log.error(e);
    ...
    public int updateByKey(Parkinfo_partner bean, String TABLENAME2) {
        try{
            String sql;
            sql = "UPDATE "+TABLENAME2+" SET pi_id=:pi_id,pu_id=:pu_id,pu_nd=:pu_nd,ctime=:ctime,utime=:utime,is_effect=:is_effect,area_code=:area_code,note=:note WHERE id=:id";
            SqlParameterSource ps = new BeanPropertySqlParameterSource(bean);
            return _np.update(sql, ps);
        }catch(Exception e){
            log.error(e);
    ...
    public int[] updateByKey (final List<Parkinfo_partner> beans, String TABLENAME2) throws SQLException{
        try{
            String sql;
            sql = "UPDATE "+TABLENAME2+" SET pi_id=?,pu_id=?,pu_nd=?,ctime=?,utime=?,is_effect=?,area_code=?,note=? WHERE id=?";
            return _np.getJdbcOperations().batchUpdate(sql, new BatchPreparedStatementSetter() {
                //@Override
                public int getBatchSize() {
                    return beans.size();
