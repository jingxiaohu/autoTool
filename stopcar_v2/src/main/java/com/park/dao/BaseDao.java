...
public class BaseDao {
	...
	* @throws
	...
	public int countByField(String TABLENAME, String FieldName,Object FieldValue) {
		String sql;
		try {
			sql = "SELECT COUNT(*) FROM " + TABLENAME + " WHERE %s = :%s";
			sql = String.format(sql, FieldName, FieldName);
			Map<String,Object> params = new HashMap<String,Object>();
			params.put(FieldName, FieldValue);
			return this._np.getJdbcOperations().queryForInt(sql, params);
		} catch (Exception e) {
			e.printStackTrace();
			log.error("BaseDao countByField", e);
			return 0;
		}
	}
	/**
	...
	public int countByCondition(String sql) {
		try {
			return this._np.getJdbcOperations().queryForInt(sql);
		} catch (Exception e) {
			e.printStackTrace();
			log.error("BaseDao countByCondition", e);
			return 0;
		}
	}
	/**
