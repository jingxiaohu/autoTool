...
public class Park_couponDao extends BaseDao{
    ...
    Logger log = Logger.getLogger(Park_couponDao.class);



    public  String TABLE = "park_coupon";

    public  String TABLENAME = "park_coupon";
    ...
    public  String[] carrays ={"id","name","money","discount","high_money","end_time","use_range","pc_type","ctime","utime","note"};
    public  String coulmns ="id,name,money,discount,high_money,end_time,use_range,pc_type,ctime,utime,note";
    public  String coulmns2 ="name,money,discount,high_money,end_time,use_range,pc_type,ctime,utime,note";
    ...
    public int insert(Park_coupon bean, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (name,money,discount,high_money,end_time,use_range,pc_type,ctime,utime,note) VALUES (:name,:money,:discount,:high_money,:end_time,:use_range,:pc_type,:ctime,:utime,:note)";
    ...
    //添加数据
    public int insert2(Park_coupon bean) throws SQLException{
        return insert2(bean, TABLENAME);
    }

    //添加数据
    public int insert2(Park_coupon bean, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (id,name,money,discount,high_money,end_time,use_range,pc_type,ctime,utime,note) VALUES (:id,:name,:money,:discount,:high_money,:end_time,:use_range,:pc_type,:ctime,:utime,:note)";
            SqlParameterSource ps = new BeanPropertySqlParameterSource(bean);
    ...
    public int[] insert(final List<Park_coupon> beans, String TABLENAME2) throws SQLException{
        String sql;
        try{
            sql = "INSERT INTO "+TABLENAME2+" (name,money,discount,high_money,end_time,use_range,pc_type,ctime,utime,note) VALUES (?,?,?,?,?,?,?,?,?,?)";
    ...
    public List<Park_coupon> selectAll(String TABLENAME2) {
        String sql;
        try{
            sql = "SELECT id,name,money,discount,high_money,end_time,use_range,pc_type,ctime,utime,note FROM "+TABLENAME2+" ORDER BY id";
    ...
    public List<Park_coupon> selectLast(int num ,String TABLENAME2) {
        String sql;
        try{
            sql = "SELECT id,name,money,discount,high_money,end_time,use_range,pc_type,ctime,utime,note FROM "+TABLENAME2+" ORDER BY id DESC LIMIT "+num+"" ;
            return _np.getJdbcOperations().query(sql, new BeanPropertyRowMapper<Park_coupon>(Park_coupon.class));
        }catch(Exception e){
            //createTable(TABLENAME2);
            log.error(e);
    ...
    public List<Park_coupon> selectGtKey(long id, String TABLENAME2) {
        String sql;
        try{
            sql="SELECT id,name,money,discount,high_money,end_time,use_range,pc_type,ctime,utime,note FROM "+TABLENAME2+" WHERE id>:id";
            Map<String,Object> param = new HashMap<String,Object>();
            param.put("id", id);
    ...
    public Park_coupon selectByKey(long id, String TABLENAME2) {
        ...
        try{
            ...
            Map<String,Object> param = new HashMap<String,Object>();
            param.put("id", id);
            return (Park_coupon)_np.queryForObject(sql, param, new BeanPropertyRowMapper<Park_coupon>(Park_coupon.class));
        }catch(Exception e){
            //createTable(TABLENAME2);
            log.error(e);
            return null;
    ...
    public List<Park_coupon> selectByPage(int begin, int num, String TABLENAME2) {
        try{
            String sql;
            sql = "SELECT id,name,money,discount,high_money,end_time,use_range,pc_type,ctime,utime,note FROM "+TABLENAME2+" LIMIT "+begin+", "+num+"";
            return _np.getJdbcOperations().query(sql,new BeanPropertyRowMapper<Park_coupon>(Park_coupon.class));
        }catch(Exception e){
            //createTable(TABLENAME2);
            log.error(e);
    ...
    public int updateByKey(Park_coupon bean, String TABLENAME2) {
        try{
            String sql;
            sql = "UPDATE "+TABLENAME2+" SET name=:name,money=:money,discount=:discount,high_money=:high_money,end_time=:end_time,use_range=:use_range,pc_type=:pc_type,ctime=:ctime,utime=:utime,note=:note WHERE id=:id";
            SqlParameterSource ps = new BeanPropertySqlParameterSource(bean);
            return _np.update(sql, ps);
        }catch(Exception e){
            log.error(e);
    ...
    public int[] updateByKey (final List<Park_coupon> beans, String TABLENAME2) throws SQLException{
        try{
            String sql;
            sql = "UPDATE "+TABLENAME2+" SET name=?,money=?,discount=?,high_money=?,end_time=?,use_range=?,pc_type=?,ctime=?,utime=?,note=? WHERE id=?";
            return _np.getJdbcOperations().batchUpdate(sql, new BatchPreparedStatementSetter() {
                //@Override
                public int getBatchSize() {
                    return beans.size();
