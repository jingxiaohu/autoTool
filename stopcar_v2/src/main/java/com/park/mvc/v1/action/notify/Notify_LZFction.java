
package com.park.mvc.v1.action.notify;

import java.util.Arrays;
import java.util.HashMap;
import java.util.Iterator;
import java.util.List;
import java.util.Map;

import org.apache.struts2.convention.annotation.Action;

import CCBSign.RSASig;

import com.park.bean.ReturnDataNew;
import com.park.mvc.v1.action.BaseV1Action;

/**
 * 建行龙支付WEB通知监听接口
 * @author jingxiaohu
 *
 */
public class Notify_LZFction extends BaseV1Action {
	private static String pub = "";

	/**
	* @Fields serialVersionUID : TODO(用一句话描述这个变量表示什么)
	*/ 
	private static final long serialVersionUID = -3599663972160625262L;

	/**
	 * 建行龙支付WEB通知监听接口
	 * @return
	 *  QueryString=POSID=426295203&BRANCHID=510000000&ORDERID=2016112513104344&PAYMENT=0.01&CURCODE=01&REMARK1=1&REMARK2=&ACC_TYPE=02&SUCCESS=Y&TYPE=1&REFERER=&CLIENTIP=118.116.108.94&SIGN=
2016-11-25 13:10:39 [com.park.mvc.v1.action.BaseV1Action] INFO (Notify_LZFction.java:57) - notify_lzf params={SIGN=, REMARK1=1, CLIENTIP=118.116.108.94, SUCCESS=Y, REMARK2=, BRANCHID=510000000, ACC_TYPE=02, ORDERID=2016112513104344, POSID=426295203, REFERER=, PAYMENT=0.01, CURCODE=01, TYPE=1}
	 */
	@Action(value = "notify_lzf")
	public String notify_lzf(){
		ReturnDataNew returnData = new ReturnDataNew();
		//非空验证
		try {
			//获取支付宝POST过来反馈信息
			Map<String,String> params = new HashMap<String,String>();
			Map requestParams = getRequest().getParameterMap();
			for (Iterator iter = requestParams.keySet().iterator(); iter.hasNext();) {
				String name = (String) iter.next();
				String[] values = (String[]) requestParams.get(name);
				String valueStr = "";
				for (int i = 0; i < values.length; i++) {
					valueStr = (i == values.length - 1) ? valueStr + values[i]
							: valueStr + values[i] + ",";
				}
				//乱码解决，这段代码在出现乱码时使用。如果mysign和sign不相等也可以使用这段代码转化
				//valueStr = new String(valueStr.getBytes("ISO-8859-1"), "gbk");
				params.put(name, valueStr);
			}
			if(params != null){
				log.info("referer="+getRequest().getHeader("referer")); 
				log.info("QueryString="+getRequest().getQueryString());
				log.info("notify_lzf params="+params.toString());
			}
			RSASig rsaSig = new RSASig();
			rsaSig.setPublicKey(pub);//设置公钥
			String src = getRequest().getQueryString();
			List<String> list = Arrays.asList(src.split("&"));
			StringBuffer sb = new StringBuffer();
			for (int i = 0; i < list.size(); i++) {
				String str = list.get(i);
				if(!str.contains("SIGN")){
					sb.append(str).append("&");	
				}
			}
			if(sb.length() > 0){
				src = sb.substring(0, sb.length()-1);
			}
			if(rsaSig.verifySigature(params.get("SIGN"), src)){
				//验签成功
				log.info("验签成功-----------"); 
				
				if("Y".equalsIgnoreCase(params.get("SUCCESS"))){
					//是支付 还是 充值  1：充值  2：普通订单支付  3：租赁订单支付
					String REMARK1 = params.get("REMARK1");
					
					//验证成功  更新该订单的支付状态 并把对应的金额添加给用户
					String orderid = params.get("ORDERID");
					//金额
					double PAYMENT = Double.parseDouble(params.get("PAYMENT"));
					long money = (long) (PAYMENT*100);
					try {
						userPayBiz.notify_lzf(returnData,orderid,REMARK1,money);
						if(returnData != null && "0".equalsIgnoreCase(returnData.getErrorno())){
							sendRespHtml(SUCCESS);
							return null;
						}
					} catch (Exception e) {
						// TODO Auto-generated catch block
						log.error("更新该订单的支付状态 并把对应的金额添加给用户 失败", e); 
					}
					sendRespHtml(ERROR);
					return null;
				}
			}
			log.info("验签失败------error-----"); 
			sendRespHtml(ERROR);
			return null;
		} catch (Exception e) {
			log.error("Notify_LZFction.notify_lzf is error 龙支付通知监听接口（GAMESDK-JAVA）- P",e);
			sendRespHtml(ERROR);
			return null; 
		}
	}
	
	
	
	
/********************接收参数区*************************/
	


/*public static void main(String[] args) {
	String sign = "";
	RSASig rsaSig = new RSASig();
	rsaSig.setPublicKey(pub);//设置公钥
	String src = "POSID=426295203&BRANCHID=510000000&ORDERID=2016111817363972&PAYMENT=0.01&CURCODE=01&REMARK1=&REMARK2=&ACC_TYPE=02&SUCCESS=Y&TYPE=1&REFERER=&CLIENTIP=218.88.30.86";
	System.out.println(rsaSig.generateSigature(src));
	
	boolean flag = rsaSig.verifySigature(sign, src);
	System.out.println("flag="+flag);
} */


	
	

	




}
