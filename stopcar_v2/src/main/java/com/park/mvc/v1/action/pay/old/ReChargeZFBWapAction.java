...
*
 */
public class ReChargeZFBWapAction extends BaseV1Action {
	...
	* @throws
	 */
	@Action(value = "recharge_zfb_wap")
	public String recharge_zfb_wap(){
		...
		if(RequestUtil.checkObjectBlank(subject)){
			//prop_name不能为空
			returnData.setReturnData(errorcode_param, "prop_name不能为空", "");
			sendResp(returnData);
			return null;
		}
		//MD5验证
		...
		try {
			...
			subject = returnUrlDecode(subject);
			cpkeyword = returnUrlDecode(cpkeyword);
			ssid = returnUrlDecode(ssid);
			returnData = gameSdkBiz.ReChargeAction_ReCharge(returnData,pay_type,ui_id,pay_price,version,terminal_type,subject,imei,t,ssid,ip,token);
...
//			        String body = new String(getRequest().getParameter("WIDbody").getBytes("ISO-8859-1"),"UTF-8");
					//////////////////////////////////////////////////////////////////////////////////
					//把请求参数打包成数组
			        double total_fee_ = (pay_price*1.0)/100;
...
//					String sHtmlText = AlipaySubmit.buildRequest(sParaTemp,"get","确认");
	...
	/********************接收参数区*************************/
	public int pay_type;//0:无来源 1：支付宝 2：财付通 3：银联
	public int pay_price;//充值金额 单位 分
	public String version;//当前版本
	public int terminal_type;//终端类型：0:web 1:PC方式;wap 2手机WAP 3mobile：手机客户端应用方式(默认)
	public String subject;//商品名称
	public String imei;//手机唯一标识符
	public long t;//时间戳ms
	public String ssid;//WIFI名称
	public String token;//令牌
	public String cpkeyword;//透传参数
	//收银台页面上，商品展示的超链接，必填
