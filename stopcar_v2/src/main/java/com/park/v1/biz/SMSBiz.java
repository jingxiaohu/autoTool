...
//	protected MailBiz mailBiz;
	...
	public void ReturnReSendSMg(ReturnDataNew returnData, int dtype,
		...
		try {
			...
			Sms_validate bsv = getMySelfService().queryUniqueT(sql, Sms_validate.class, telephone,verify_list,vclass);
			if(bsv == null){
...
//	}
	 ...
	 * @throws Exception 
	 */
	public User_info gainUserInfoByEmail(String email) throws Exception{   
		//获取该串号的物料码对应的SD卡是否存在
		try {
			String sql = "select * from user_info where ui_email=? limit 1";
			return getMySelfService().queryUniqueT(sql, User_info.class, email);
		} catch (Exception e) {
			// TODO Auto-generated catch block
			log.error("gainUserInfoByEmail(String email)  telephone="+email+"  is error", e);
			throw new Exception("gainUserInfoByTelePhone 获取用户信息失败",e);
		}
	}
